  ...
  useEffect(() => {
   ...
   const BASE_URL = import.meta.env.VITE_API_URL;
      try {
        ...
        if (!token) {
          navigate('/userAuth'); // Redirect to login if no token
          return;
        }
        ...
        // Update user stats with API data based on the server response format
        setUserStats({
        ...
        // Update recent feedback from the API response
      ...
      <div className="w-full max-w-4xl mx-auto px-4 py-6">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-blue-600 text-xl font-medium mb-2">
            Welcome back, {userStats.fullName}!
          </h1>
        </div>
          ...
          {recentFeedback.length > 0 ? (
            <div className="space-y-4">
              {recentFeedback.map((feedback, index) => (
                <div key={index} className="flex items-center justify-between bg-blue-50 rounded-sm px-4 py-3 border-b border-gray-100 last:border-b-0">
