export default function QRGenerator() {
  ...
  const editQR = async (code) => {
    ...
    try {
      ...
      const qrTypeMap = JSON.parse(localStorage.getItem("qrTypeMap") || "{}");
      setQrType(qrTypeMap[qrData.id] || (qrData.type === "Product" ? "product" : qrData.type === "Service" ? "service" : "general"));
      setQrName(qrData.label || "");
      setDescription(qrData.description || "");
      setTags(Array.isArray(qrData.qrcode_tags) ? qrData.qrcode_tags : []);
      setEditingId(qrData.id);
      const constructedScanUrl = `${import.meta.env.VITE_SCAN_URL}/qr/${businessId}/${qrData.id}`;
      setGeneratedQrData({
        ...qrData,
